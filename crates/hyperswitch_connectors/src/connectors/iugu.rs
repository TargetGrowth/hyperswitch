...
use crate::connectors::iugu::transformers::{
    IuguAuthType, IuguChargeRequest, IuguInvoiceRequest, IuguInvoiceResponse, 
    IuguErrorResponse, IuguRouterData, IuguRefundRequest, RefundResponse
};
...
impl ConnectorCommon for Iugu {
    ...
    fn get_auth_header(
        ...
        let auth = IuguAuthType::try_from(auth_type)
    ...
    fn build_error_response(
        ...
        let response: IuguErrorResponse = res
            .response
...
impl ConnectorIntegration<Authorize, PaymentsAuthorizeData, PaymentsResponseData> for Iugu {
    ...
    fn get_url(
        ...
        match &req.request.payment_method_data {
            PaymentMethodData::Card(_) => Ok(format!("{}/charge", base)),
            PaymentMethodData::BankRedirect(_) | PaymentMethodData::BankTransfer(_) | PaymentMethodData::Wallet(_) => Ok(format!("{}/invoices", base)),
    ...
    fn get_request_body(
        ...
        match &req.request.payment_method_data {
            PaymentMethodData::Card(_) => {
                let connector_req = IuguChargeRequest::try_from(req)?;
                Ok(RequestContent::Json(Box::new(connector_req)))
            }
            PaymentMethodData::BankRedirect(_) | PaymentMethodData::BankTransfer(_) | PaymentMethodData::Wallet(_) => {
                let connector_req = IuguInvoiceRequest::try_from(req)?;
                Ok(RequestContent::Json(Box::new(connector_req)))
            }
            _ => Err(errors::ConnectorError::NotImplemented("Payment method not supported".to_string()).into()),
        }
    ...
    fn handle_response(
        ...
        let response: IuguInvoiceResponse = res
            .response
        ...
        let resource_id = ResponseId::ConnectorTransactionId(response.id.clone());
        let redirection_data = response.secure_url.as_ref().map(|url| {
...
impl ConnectorIntegration<PSync, PaymentsSyncData, PaymentsResponseData> for Iugu {
    ...
    fn get_url(
        ...
        let base = self.base_url(connectors);
        let invoice_id = req.request.connector_transaction_id.get_connector_transaction_id()
            .change_context(errors::ConnectorError::MissingRequiredField { field_name: "connector_transaction_id" })?;
...
impl ConnectorIntegration<Capture, PaymentsCaptureData, PaymentsResponseData> for Iugu {
    ...
    fn get_url(
        ...
        Ok(format!("{}/invoices/{}/capture", base, invoice_id))
    }
    ...
    fn get_request_body(
        ...
        // Iugu capture endpoint doesn't require a request body
...
impl ConnectorIntegration<Void, PaymentsCancelData, PaymentsResponseData> for Iugu {
    ...
    fn get_url(
        ...
        let base = self.base_url(connectors);
        let invoice_id = req.request.connector_transaction_id.clone();
        Ok(format!("{}/invoices/{}/cancel", base, invoice_id))
    }
    ...
    fn get_request_body(
        ...
        // Iugu cancel endpoint doesn't require a request body
        Ok(RequestContent::Json(Box::new(serde_json::Value::Null)))
    }
    ...
    fn handle_response(
        ...
        let response: IuguInvoiceResponse = res
            .response
            .parse_struct("Iugu PaymentsCancelResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
